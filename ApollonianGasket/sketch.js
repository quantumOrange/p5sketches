...
function draw() {
  ...
  while(circles.length<1000 && n<8){
    n++;
    var incompleteCircles = circles.filter(function(x) {return ( x.tangentCircles.length>0  && x.tangentCircles.length<5)});
...
function apollonian(c){
  ...
  var r_min = 3.0;
  var c23 = decartes(c,c2,c3).filter(function(x) { return (!c1.isEqual(x) && x.r > r_min)})
  var c13 = decartes(c,c1,c3).filter(function(x) { return (!c2.isEqual(x) && x.r > r_min)});
  var c12 = decartes(c,c1,c2).filter(function(x) { return (!c3.isEqual(x) && x.r > r_min)});
